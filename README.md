...
- **工具系统** — 30 个内置工具（文件读写编辑、Shell、搜索、Web 抓取/搜索、任务管理、子 Agent 派发、用户交互、规划模式、会话管理、自查等）+ MCP 协议兼容；搜索引擎支持 Brave Search API（需配置 key）或 Bing 免费爬取；工具结果自动截断（30k 字符硬上限）
- **记忆管理** — 短期对话历史 + 智能上下文压缩（LLM Task-based 结构化摘要）+ 长期结构化记忆（用户画像/客观事实/经验教训，需配置 Embedding）
- **上下文注入** — `soul.md`（人格）/ `agents.md`（指令）/ `memory.md`（记忆）/ Skills 目录（渐进式披露）/ 动态环境信息（日期、平台、模型）启动时注入，watchdog 实时热更新
- **编排引擎** — think-act-observe Agent Loop + 工具式子 Agent（`spawn_agent` / `resume_agent`），支持并行工具执行、后台 Agent（`run_in_background`）、预定义 Agent 类型（explore/code/research）、完全隔离、工具白名单、轮次中断恢复；Tool Result 后处理引擎（条件性注入框架提示）
...
- **定时任务** — Cron 调度（`/cron` 命令或模型自主调用），支持 recurring / once，独立上下文可跨执行追踪状态变化
- **多模态输入** — 对话中直接发送图片/视频/音频，支持本地文件路径和 HTTP URL；图片 base64 编码（超 20MB 自动缩放）或 URL 直传；`read_file` 支持 PDF 分页读取和图片 base64 读取
- **国际化** — 支持中文/English 双语，`/lang` 命令切换，首次运行向导语言持久化
- **权限系统** — 工具三级权限（auto_allow / require_confirm / deny），写操作和 Shell 默认需用户确认，可配置
- **CLI 产品形态** — prompt_toolkit 非全屏 TUI（固定输入 + 原生终端滚动 + 文本可选中复制）；请求排队（Agent 处理中可继续输入，队列固定显示在输入框上方）；思考中/输出中状态指示；Ctrl+C 中断当前生成；首次运行配置向导、启动时显示模型参数和上下文使用率
...
[tools.permissions]
# auto_allow = ["read_file", "glob_files", "grep_files", ...]   # 自动放行
# require_confirm = ["run_command", "write_file", "edit_file"]   # 需用户确认
# deny = []                                                       # 完全禁用
...
| `/cron "<expr>" <ttl> <prompt>` | 创建定时任务 |
| `/cron once <ttl> <prompt>` | 创建一次性定时任务 |
| `/cron list` | 查看活跃的定时任务 |
| `/cron log <id>` | 查看任务执行历史 |
| `/cron del <id>` | 删除定时任务 |
...
│       ├── agents.py   # spawn_agent/resume_agent (含后台模式)
│       ├── interaction.py # ask_user (结构化提问)
│       ├── plan.py     # enter_plan_mode/exit_plan_mode
